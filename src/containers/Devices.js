...
export default () => {
  ...
  const deviceFilter = device => {
    switch (true) {
      ...
      case (filter.os != "nezáleží" || filter.os != undefined) &&
        (filter.vendor != "nezáleží" || filter.vendor != undefined):
        return (
          device.vendor.toLowerCase() === filter.vendor &&
          device.os.toLowerCase() === filter.os
        );
      default:
        return null;
    }
  ...
  const vendorOptions = vendor.map((voption, i) => (
    <option key={i} value={voption}>
      {voption}
    </option>
  ));
  const osOptions = os.map((osOption, i) => (
    <option key={i} value={osOption}>
      {osOption}
    </option>
  ));
  ...
  return (
    <DevicesSection>
      ...
      <article className='devices'>{visibleDevices}</article>
    </DevicesSection>
  );
};
